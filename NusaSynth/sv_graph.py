...
def sv_run(state: SVState) -> dict:
    ...
    def process_group(group: list[SentenceRecord]) -> list[SentenceRecord]:
        ...
        def invoke_with_parse_retry() -> SVOutput | None:
            ...
            for attempt in range(total_attempts):
                try:
                    ...
                    if attempt < PARSE_MAX_RETRY:
                        print(f"SV WARNING seed_id={seed_id}: parse failed (attempt {attempt + 1}/{total_attempts}: {type(e).__name__}). Coba lagi.")
                    else:
                        print(f"SV FAILED seed_id={seed_id}: parse failed setelah {total_attempts} attempts ({type(e).__name__}). Group ditandai VALIDATION_FAILED.")
            return None
        ...
        if result_or_none is None:
            for sent in group:
                ...
                sent["sv_feedback"] = "sv_parse_failure"
            return group
        result: SVOutput = result_or_none
